<?php 
// -> Generates an .ics file to add to your calendar, quickly.
/****************************************\
|*										*|
|*          @TellieBot source           *|
...
$result = $db->query("SELECT * FROM DatePickerDates WHERE id='".$db->escape_string($picker['finaldate'])."' LIMIT 1");
$option = $result->fetch_assoc();

	$e->startDate =date("Ymd",$option['datestamp']);
	$e->uri = 'http://telliebot.me';
	$e->location = 'TellieBot';
	$e->description = "Planned by {$picker['user_firstname']} using Telliebot.";
	$e->summary = ucfirst($picker['name']);
});
echo $cal->serialize();
